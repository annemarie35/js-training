...
export function testBasics() {
  ...
  const isEven :(num: number) => boolean = (num: number) => num % 2 === 0;
  ...
  const uniqueId: unique symbol = Symbol("id"); // Unique and immutable identifier
  // @ts-ignore
...
export function testGenericsTypes() {
   ...
   */
  function parseJSON<Type>(input: string): Type {
   ...
   */
  function addProp<Type extends Record<string, string>>(input: Type): Type & { newProp: string }
  {
   ...
   */
  function transform<Input, Output>(input: Input, fn: (input: Input) => Output): Output {
      return fn(input)
  }
  function stringify(data: any): string {
      return String(data)
  }
  function length(data: string): number {
      return data.length
  }
...
export function testLiteralTypes() {
  ...
  //${FirstName} ${LastName} utilise les Template Literal Types de TypeScript
  // Cela crée automatiquement toutes les combinaisons possibles entre FirstName et LastName
  ...
  type ExtractFeeling<Statement> =
    Statement extends `${string} ${infer feeling} ${string}`
      ? feeling
      : "";
